...
async function recordAction(userId, actionType, reward) {
    await supabase.from('mining_actions').insert([{ telegram_id: userId, action_type: actionType, reward_amount: reward }]);
...
bot.start(async (ctx) => {
    ...
    if (!user || !user.is_registered) {
        ctx.reply('مرحباً بك! لإتمام التسجيل والحصول على مكافأتك الأولى، يرجى القيام بالتالي:', Markup.inlineKeyboard([
            ...
            [Markup.button.callback('تمت المهام الإجبارية', 'check_registration')]
        ]));
    } else {
        ctx.reply('أهلاً بك في منصة Obsidian Genesis. اختر الخدمة:', Markup.inlineKeyboard([
            [Markup.button.callback('⛏ تعدين $OBSD', 'mine_menu')],
            [Markup.button.url('🛒 شراء $OBSD (QuickSwap)', 'https://dapp.quickswap.exchange/swap?type=best&from=ETH&to=0x2a2C206aC686eDD7D5b8Cf1cf325dE5261cD446F')],
...
bot.action('check_registration', async (ctx) => {
    ...
    ctx.reply('تم التحقق! أنت الآن مسجل رسمياً. يمكنك الآن البدء بالتعدين.');
});
